...
export default function ToolItem({ name, desc, color, tag, link }: ToolItemProps) {
  return (
    <a href={link} target="_blank" rel="noreferrer" className="block group relative border-b border-white/20 py-8 md:py-12 cursor-pointer hover:bg-white/5 transition-colors duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between px-4 md:px-8">
        <div className="flex-1">
          ...
          <div className="flex gap-4 items-center">
            <span className={`px-3 py-1 text-xs font-bold uppercase tracking-widest text-[#151515] ${color}`}>
              {tag}
            </span>
